...
class LTIClient:
    ...
    def __init__(self, config: LTIConfig):
        if not HAS_DEPENDENCIES:
            raise LTIConfigurationError("Required dependencies (PyJWT, requests) not installed")
    ...
    def _get_lineitem_path(self, course_id: str, assignment_id: str = None) -> str:
        if assignment_id and (assignment_id.startswith('http://') or assignment_id.startswith('https://')):
    ...
    def create_assignment(self,
                          ...
                          submission_end_date_time: str = None) -> Assignment:
        url = self._get_lineitem_path(course_id)
        ...
        assignment_data = self._make_authenticated_request('post', url, content_type, payload)
    ...
    def update_assignment(self,
        ...
        assignment_data = self._make_authenticated_request('put', url, content_type, payload)
    ...
    def submit_score(self,
                     ...
                     grading_progress: str = 'FullyGraded') -> Dict[str, Any]:
        ...
        logger.debug(f"Submitting score {score} for user {user_id} in assignment {assignment_id}")
    ...
    def get_scores(self, course_id: str, assignment_id: str, user_id: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Retrieved {len(scores) if isinstance(scores, list) else 'unknown'} scores")
            return scores
    ...
    def get_assignment(self, course_id: str, assignment_id: str) -> Assignment:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise LTIRequestError(f"Assignment retrieval error: {error_details.get('reason', 'Unknown error')}")
            else:
                response.raise_for_status()
    ...
    def get_memberships(self, course_id: str, role_filter: Optional[str] = None) -> List[Member]:
        ...
        try:
            ...
            if 'pageOf' in data and 'membershipSubject' in data['pageOf']:
                memberships_data = data['pageOf']['membershipSubject'].get('membership', [])
            ...
            for member_data in memberships_data:
                ...
                if role_filter:
                    if any(role_filter.lower() in role.lower() for role in member.roles):
